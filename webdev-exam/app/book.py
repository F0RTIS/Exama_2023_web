...
from app import db
# параметры для редактирования книг кроме жанров
...
def new_book():
    if request.method == "POST":
        ...
        if not short_desc:
                flash("При добавлении книги произошла ошибка 2", "danger")
        ...
        if f and f.filename:
            ...
            flash("При добавлении книги произошла ошибка 1", "danger")
            return error_create(genres_list_int, book_params)

    #   Проверка на то что все поля есть
        for param in LIST_PARAMS:
            if not book_params[param]:
                flash("При добавлении книги произошла ошибка 3", "danger")
                return error_create(genres_list_int, book_params)

        if not genres_list_int:
            flash("При добавлении книги произошла ошибка 4", "danger")
            return error_create(genres_list_int, book_params)
        ...
        with db.connection.cursor(named_tuple = True) as cursor:
            try:
                ...
                db.connection.rollback()
                flash("При добавлении произошла ошибка 5", "danger")
                return error_create(genres_list_int, book_params)
